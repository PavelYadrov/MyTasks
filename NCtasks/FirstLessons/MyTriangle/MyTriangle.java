...
import com.task6.pavelyadrov.TriangleType;
...
public class MyTriangle {
    ...
    public String getType(){
        ...
        double secondSide = v2.distance(v3);
        double thirdSide = v3.distance(v1);
        if (Math.abs(firstSide-secondSide)<=precision && Math.abs(secondSide-thirdSide)<=precision) {
            type = TriangleType.EQUILATERAL.getName();
        }
        else if (Math.abs(firstSide-secondSide)<=precision || Math.abs(secondSide-thirdSide)<=precision || Math.abs(firstSide-thirdSide)<=precision) {
            type = TriangleType.ISOSCELES.getName();
        }
        else type = TriangleType.SCALENE.getName();
        return type;
    }
    ...
    public boolean equals(Object o) {
        ...
        for (int k = 0; k < 3; k++) {
            for (int j=0;j<firstTriangle.length;j++) {
                if (Math.abs(secondTriangle[k]-firstTriangle[j])<=precision) {
                    firstTriangle[j]=0.0;
                    count++;
                    break;
                }
